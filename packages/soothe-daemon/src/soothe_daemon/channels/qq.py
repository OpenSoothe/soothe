...
class QQConfig:
    ...
    # Optional: directory to save inbound attachments. If empty, use default.
    media_dir: str = ""
...
def _make_bot_class(channel: QQChannel) -> type[botpy.Client]:
    ...
    class _Bot(botpy.Client):
        def __init__(self):
            # Disable botpy's file log — soothe uses standard logging
...
class QQChannel(Channel):
    ...
    def _init_media_root(self) -> Path:
        ...
        if self.config.media_dir:
            ...
            root = Path.home() / ".soothe" / "media" / "qq"

        root.mkdir(parents=True, exist_ok=True)
        logger.info("QQ media directory: %s", str(root))
        return root

    async def start(self) -> None:
        """Start the QQ bot with auto-reconnect loop."""
        if not QQ_AVAILABLE:
            logger.error("SDK not installed. Run: pip install qq-botpy")
            return

        if not self.config.app_id or not self.config.secret:
            logger.error("app_id and secret not configured")
            return
    ...
    async def send(self, chat_id: str, message: ChannelMessage) -> None:
        ...
        try:
            ...
            logger.exception("Error sending message to chat_id=%s", chat_id)
    ...
    async def _send_media(
        ...
        try:
            ...
            logger.warning("QQ send media network error filename=%s err=%s", filename, e)
            raise
        except Exception:
            # API-level or other non-network errors — return False so send() can fallback
            logger.exception("QQ send media failed filename=%s", filename)
            return False
        ...
        if not media_ref.startswith("http://") and not media_ref.startswith("https://"):
            try:
                ...
                if not local_path.is_file():
                    logger.warning("QQ outbound media file not found: %s", str(local_path))
                    return None, None

                data = await asyncio.to_thread(local_path.read_bytes)
                return data, local_path.name
            except Exception as e:
                logger.warning("QQ outbound media read error ref=%s err=%s", media_ref, e)
                return None, None

        # Remote URL - download without validation
        if not self._http:
            self._http = aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=120))
        try:
            async with self._http.get(media_ref, allow_redirects=True) as resp:
                if resp.status >= 400:
                    logger.warning(
                        "QQ outbound media download failed status=%s url=%s",
                        resp.status,
                        media_ref,
                    )
                    return None, None
            ...
            logger.warning("QQ outbound media download error url=%s err=%s", media_ref, e)
            return None, None
    ...
    async def _on_message(self, data: C2CMessage | GroupMessage, is_group: bool = False) -> None:
        """Parse inbound message, download attachments, and publish to the manager."""
        try:
            if is_group:
            ...
            if self.config.ack_message:
                try:
                    ...
                    logger.debug("QQ ack message failed for chat_id=%s", chat_id)
            ...
            logger.exception("Error handling inbound QQ message id=%s", getattr(data, "id", "?"))
    ...
    async def _handle_attachments(
        ...
        for att in attachments:
            ...
            logger.info("QQ downloading file: %s", filename or url)
    ...
    async def _download_to_media_dir_chunked(
        ...
        try:
            async with self._http.get(
                ...
                if resp.status != 200:
                    logger.warning("QQ download failed: status=%s url=%s", resp.status, url)
                    return None
                ...
                try:
                    async for chunk in resp.content.iter_chunked(chunk_size):
                        ...
                        if downloaded > max_bytes:
                            logger.warning(
                                "QQ download exceeded max_bytes=%s url=%s -> abort",
                                max_bytes,
                                url,
                            )
                            return None
                ...
                logger.info("QQ file saved: %s", str(target))
                return str(target)

        except Exception:
            logger.exception("QQ download error")
            return None
        finally:
