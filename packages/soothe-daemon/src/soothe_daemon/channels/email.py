...
class EmailConfig:
    ...
    max_attachment_size: int = 2_000_000
...
class EmailChannel(Channel):
    """Email channel using IMAP polling for inbound + SMTP for outbound.
    ...
    Outbound:
    - Send responses via SMTP back to sender address
    """
    ...
    async def start(self) -> None:
        ...
        if not self.config.verify_dkim and not self.config.verify_spf:
            logger.warning(
                "[Email] DKIM and SPF verification are both DISABLED. "
                "Emails with spoofed From headers will be accepted."
            )
        logger.info("[Email] Starting channel (IMAP polling mode)...")
    ...
    async def send(self, chat_id: str, message: ChannelMessage) -> None:
        """Send email via SMTP."""
        if not self.config.consent_granted:
            logger.warning("[Email] Skip send: consent_granted is false")
            return
        ...
        force_send = bool(message.metadata.get("force_send"))

        if is_reply and not self.config.auto_reply_enabled and not force_send:
            logger.info("[Email] Skip automatic reply to %s: auto_reply_enabled is false", to_addr)
            return

        base_subject = self._last_subject_by_chat.get(to_addr, "soothe reply")
        subject = self._reply_subject(base_subject)
        if message.metadata and isinstance(message.metadata.get("subject"), str):
            override = message.metadata["subject"].strip()
            if override:
                subject = override
        ...
        try:
            ...
            logger.exception("[Email] Error sending to %s", to_addr)
            raise
        ...
        for attempt in range(2):
            try:
                ...
                if attempt == 1 or not self._is_stale_imap_error(exc):
                    raise
                logger.warning("[Email] IMAP connection went stale, retrying once: %s", exc)
        ...
        try:
            ...
            try:
                ...
                if self._is_missing_mailbox_error(exc):
                    logger.warning(
                        "[Email] Mailbox unavailable, skipping poll for %s: %s", mailbox, exc
                    )
                    return messages
                raise
            if status != "OK":
                logger.warning(
                    "[Email] Mailbox select returned %s, skipping poll for %s", status, mailbox
                )
                return messages
            ...
            for imap_id in ids:
                ...
                if self._is_self_address(sender):
                    logger.info("[Email] From %s ignored: matches bot-owned address", sender)
                ...
                if self.config.verify_spf and not spf_pass:
                    logger.warning(
                        "[Email] From %s rejected: SPF verification failed",
                        sender,
                    )
                    self._remember_processed_uid(uid, dedupe, cycle_uids)
                    continue
                if self.config.verify_dkim and not dkim_pass:
                    logger.warning(
                        "[Email] From %s rejected: DKIM verification failed",
                        sender,
                    )
                    self._remember_processed_uid(uid, dedupe, cycle_uids)
                    continue
    ...
    def _extract_attachments(
        ...
        *,
        ...
        saved: list[Path] = []
        media_dir = Path.home() / ".soothe" / "media" / "email"
        ...
        for part in msg.walk():
            ...
            if not any(fnmatch(content_type, pat) for pat in allowed_types):
                logger.debug(
                    "[Email] Attachment skipped (type %s): not in allowed list", content_type
                )
                continue
            ...
            if len(payload) > max_size:
                logger.warning(
                    "[Email] Attachment skipped: size %s exceeds limit %s",
                    len(payload),
                    max_size,
                )
                continue
            ...
            try:
                ...
                saved.append(dest)
                logger.info("[Email] Attachment saved: %s", dest)
            except Exception as exc:
                logger.warning("[Email] Failed to save attachment %s: %s", dest, exc)
    ...
    def _reply_subject(self, base_subject: str) -> str:
        subject = (base_subject or "").strip() or "soothe reply"
