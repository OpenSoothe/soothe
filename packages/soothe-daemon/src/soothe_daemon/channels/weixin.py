...
class WeixinConfig:
    """Personal WeChat channel configuration (Pydantic-like for soothe)."""
    ...
    state_dir: str = ""  # Default: ~/.soothe/weixin/
...
class WeixinChannel(Channel):
    ...
    supports_streaming = False  # WeChat iLink does not support native streaming
    ...
    def _get_state_dir(self) -> Path:
        ...
        if self.config.state_dir:
            ...
            d = Path.home() / ".soothe" / "weixin"
        d.mkdir(parents=True, exist_ok=True)
        self._state_dir = d
        return d

    def _get_media_dir(self) -> Path:
        """Get media directory for Weixin downloads."""
        media_dir = Path.home() / ".soothe" / "media" / "weixin"
    ...
    def _load_state(self) -> bool:
        ...
        try:
            ...
            logger.exception("[Weixin] Failed to load account state")
            return False
            ...
            while self._running:
                ...
                if status == "confirmed":
                    ...
                    if token:
                        ...
                        self._save_state()
                        logger.info(
                            "[Weixin] login successful! bot_id=%s user_id=%s",
                            bot_id,
                            user_id,
                        )
                        return True
                    else:
                        logger.error("[Weixin] Login confirmed but no bot_token in response")
                        return False
                    ...
                    if refresh_count > MAX_QR_REFRESH_COUNT:
                        logger.warning(
                            "[Weixin] QR code expired too many times (%d/%d), giving up.",
                            refresh_count - 1,
    ...
    async def start(self) -> None:
        ...
        if self.config.token:
            ...
            if not await self._qr_login():
                logger.error(
                    "[Weixin] login failed. Run 'soothe channels login weixin' to authenticate."
                )
                self._running = False
                return

        logger.info("[Weixin] channel starting with long-poll...")
        ...
        while self._running:
            try:
                ...
                if not self._running:
                    break
                logger.exception("[Weixin] poll loop error")
    ...
    async def _poll_once(self) -> None:
        ...
        if is_error:
            if errcode == ERRCODE_SESSION_EXPIRED or ret == ERRCODE_SESSION_EXPIRED:
                ...
                remaining = self._session_pause_remaining_s()
                logger.warning(
                    "[Weixin] session expired (errcode %d). Pausing %d min.",
                    errcode,
        ...
        for msg in msgs:
            try:
                ...
                logger.exception("[Weixin] Failed to process message")
    ...
    async def _process_message(self, msg: dict) -> None:
        ...
        logger.info(
            "[Weixin] inbound: from=%s items=%s bodyLen=%d",
            from_user_id,
        ...
        try:
            ...
            for idx, (download_source, cdn_url) in enumerate(download_candidates):
                try:
                    ...
                    if should_fallback:
                        logger.warning(
                            "[Weixin] media download failed via full_url, falling back to encrypt_query_param: type=%s err=%s",
                            media_type,
                            e,
                        )
                        continue
            ...
            logger.exception("[Weixin] Error downloading media")
            return None
    ...
    async def _refresh_context_token_if_stale(self, chat_id: str, context_token: str) -> str:
        ...
        logger.debug(
            "[Weixin] context_token for %s is %.0fs old; refreshing via getconfig",
            chat_id,
            age,
        )
        ...
        try:
            ...
            logger.warning("[Weixin] getconfig failed for %s: %s", chat_id, e)
            return context_token

        if data.get("ret", 0) != 0:
            logger.warning(
                "[Weixin] getconfig returned ret=%d for %s: %s",
                data.get("ret"),
                chat_id,
                data.get("errmsg", ""),
            )
        ...
        if new_token and new_token != context_token:
            logger.info(
                "[Weixin] context_token refreshed for %s (age %.0fs -> fresh)",
                chat_id,
                age,
            )
    ...
    async def _flush_tool_hints(self, chat_id: str) -> None:
        ...
        logger.info(
            "[Weixin] Flushing %d buffered tool hint(s) for %s",
            len(hints),
            chat_id,
        )
        ...
        if not ctx_token:
            logger.warning(
                "[Weixin] Dropped %d buffered tool hint(s) for %s: no context_token",
                len(hints),
                chat_id,
            )
            return
        ...
        try:
            ...
            logger.exception("[Weixin] Failed to flush buffered tool hints for %s", chat_id)
    ...
    async def send(self, chat_id: str, message: ChannelMessage) -> None:
        ...
        if is_progress and message.is_tool_hint():
            if not self.send_tool_hints:
                return
            self._pending_tool_hints.setdefault(chat_id, []).append(message.content)
            logger.debug(
                "[Weixin] Buffered tool hint for %s (count=%d)",
                chat_id,
                len(self._pending_tool_hints[chat_id]),
            )
            return

        # Reasoning deltas are invisible in WeChat (there is no reasoning
        # UI).  Skip them entirely — do not send and do not flush buffer.
        if is_progress and message.metadata.get("_reasoning_delta"):
            logger.debug("[Weixin] Dropped invisible reasoning delta for %s", chat_id)
            return
        ...
        if is_progress and not content and not (message.media or []):
            logger.debug(
                "[Weixin] Skipped empty progress message for %s (no visible content)",
                chat_id,
            )
            return
        ...
        typing_ticket = ""
        with suppress(Exception):
            typing_ticket = await self._get_typing_ticket(chat_id, ctx_token)

        if typing_ticket:
            with suppress(Exception):
                await self._send_typing(chat_id, typing_ticket, TYPING_STATUS_TYPING)

        typing_keepalive_stop = asyncio.Event()
        typing_keepalive_task: asyncio.Task | None = None
        if typing_ticket:
            typing_keepalive_task = asyncio.create_task(
                self._typing_keepalive_loop(chat_id, typing_ticket, typing_keepalive_stop)
            )
        ...
        try:
            ...
            for media_path in message.media or []:
                try:
                    ...
                    logger.warning(
                        "[Weixin] Network error sending media %s",
                        media_path,
                        exc_info=True,
                    )
                    raise
                    ...
                    if status_code >= 500:
                        ...
                        logger.exception(
                            "[Weixin] Server error (%d %s) sending media %s",
                            status_code,
                    ...
                    logger.exception("[Weixin] Failed to send media %s", media_path)
            ...
            logger.exception("[Weixin] Error sending message")
            raise
        finally:
            ...
            if typing_ticket and not is_progress:
                with suppress(Exception):
                    await self._send_typing(chat_id, typing_ticket, TYPING_STATUS_CANCEL)
    ...
    async def _start_typing(self, chat_id: str, context_token: str = "") -> None:
        ...
        try:
            ...
            logger.debug("[Weixin] typing indicator start failed for %s: %s", chat_id, e)
            return
    ...
    async def _stop_typing(self, chat_id: str, *, clear_remote: bool) -> None:
        ...
        try:
            ...
            logger.debug("[Weixin] typing clear failed for %s: %s", chat_id, e)
    ...
    async def _send_text(
        ...
        client_id = f"soothe-{uuid.uuid4().hex[:12]}"
...
# CRITICAL: Keep unchanged - protocol-critical crypto helpers
# ---------------------------------------------------------------------------
...
def _encrypt_aes_ecb(data: bytes, aes_key_b64: str) -> bytes:
    ...
    try:
        ...
        logger.warning("[Weixin] Failed to parse AES key for encryption, sending raw: %s", e)
        return data
    ...
    try:
        ...
        logger.warning("[Weixin] Cannot encrypt media: install 'pycryptodome' or 'cryptography'")
        return data
...
def _decrypt_aes_ecb(data: bytes, aes_key_b64: str) -> bytes:
    ...
    try:
        ...
        logger.warning("[Weixin] Failed to parse AES key, returning raw data: %s", e)
        return data
    ...
    if decrypted is None:
        try:
            ...
            logger.warning(
                "[Weixin] Cannot decrypt media: install 'pycryptodome' or 'cryptography'"
            )
            return data
