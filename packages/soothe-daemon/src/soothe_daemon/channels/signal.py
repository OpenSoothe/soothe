...
class SignalConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.group_message_buffer_size <= 0:
...
def _get_media_dir() -> Path:
    """Return the media directory for Signal attachments."""
    return Path.home() / ".soothe" / "media" / "signal"
...
class SignalChannel(Channel):
    ...
    def default_config(cls) -> dict[str, Any]:
        return {
            ...
            "daemon_port": 8080,
            "group_message_buffer_size": 20,
            "attachments_dir": None,
            "dm": {"enabled": False, "policy": "allowlist", "allow_from": []},
            "group": {
                "enabled": False,
                "policy": "allowlist",
                "allow_from": [],
    ...
    def __init__(self, config: SignalConfig | dict[str, Any], manager):
        if isinstance(config, dict):
            ...
            config = SignalConfig(
                ...
                daemon_host=config.get("daemon_host", "localhost"),
                daemon_port=config.get("daemon_port", 8080),
                group_message_buffer_size=config.get("group_message_buffer_size", 20),
                attachments_dir=config.get("attachments_dir"),
                dm=SignalDMConfig(
                    enabled=dm_config.get("enabled", False),
                    policy=dm_config.get("policy", "allowlist"),
                    allow_from=dm_config.get("allow_from", []),
                ),
                group=SignalGroupConfig(
                    enabled=group_config.get("enabled", False),
                    policy=group_config.get("policy", "allowlist"),
                    allow_from=group_config.get("allow_from", []),
                    require_mention=group_config.get("require_mention", True),
                ),
            )
    ...
    async def send(self, chat_id: str, message: ChannelMessage) -> None:
        ...
        is_progress_message = message.is_progress()
        try:
            plain_text, text_styles = _markdown_to_signal(message.content)
            if not plain_text and not message.media:
                return
            recipient_params = self._recipient_params(chat_id)
            ...
            for i, chunk in enumerate(chunks):
                ...
                if message.media and i == 0:
                    params["attachments"] = message.media
                ...
                if "error" in response:
                    ...
                    self.logger.debug(
                        "Signal message sent, timestamp: %s",
                        response.get("result", {}).get("timestamp"),
                    )
    ...
    async def _handle_receive_notification(self, params: dict[str, Any]) -> None:
        ...
        async with self._safe_handle("receive notification", params):
            ...
            # Handle sync messages (messages sent from another device)
                ...
                if destination:
                    self.logger.debug(
                        "Sync message sent to %s: %s",
                        destination,
                        sent_msg.get("message", "")[:50],
                    )
    ...
    async def _handle_data_message(
        ...
        if data_message.get("reaction"):
            self.logger.debug(
                "Ignoring reaction message from %s: %s",
                sender_number,
        ...
        if not allowed:
            # Group denials stay dropped; no pairing in soothe.
            return
        ...
        *,
        ...
        if self.config.dm.policy == "allowlist":
            if not self._sender_matches_allowlist(sender_id, self.config.dm.allow_from):
                self.logger.debug(
                    "Ignoring DM from %s (policy: %s)",
                    sender_id,
                    self.config.dm.policy,
                )
                return False, chat_id
        return True, chat_id
    ...
    def _add_to_group_buffer(
        ...
        self.logger.debug(
            "Added message to group buffer %s: %d/%d",
            group_id,
    ...
    async def _send_typing(
        ...
        self.logger.warning(
            "Failed to send Signal typing %s for %s: %s",
            action,
            chat_id,
            last_error,
        )
    ...
    async def _ensure_typing_indicators_enabled(self) -> None:
        ...
        if "error" in response:
            self.logger.warning(
                "Failed to enable Signal typing indicators: %s",
                response["error"],
            )
        else:
