...
# Media directory for WeCom
WECOM_MEDIA_DIR = Path.home() / ".soothe" / "media" / "wecom"
...
class WecomChannel(Channel):
    ...
    def __init__(self, config: Any, manager: Any) -> None:
        if isinstance(config, dict):
            config = WecomConfig.from_dict(config)
    ...
    async def start(self) -> None:
        ...
        if not WECOM_AVAILABLE:
            self.logger.error("SDK not installed. Run: pip install soothe-daemon")
            return
    ...
    async def _process_message(self, frame: Any, msg_type: str) -> None:
        """Process incoming message and forward to manager."""
        try:
            # Extract body from WsFrame dataclass or dict
            if hasattr(frame, "body"):
                body = frame.body or {}
    ...
    async def _download_and_save_media(
        ...
        try:
            ...
            if len(data) > WECOM_UPLOAD_MAX_BYTES:
                self.logger.warning(
                    "inbound media too large: %d bytes (max %d)",
                    len(data),
            ...
            WECOM_MEDIA_DIR.mkdir(parents=True, exist_ok=True)

            if not filename:
                filename = fname or f"{media_type}_{hash(file_url) % 100000}"
            filename = safe_filename(filename)

            file_path = WECOM_MEDIA_DIR / filename
    ...
    async def _upload_media_ws(
        ...
        try:
            ...
            if resp.errcode != 0:
                self.logger.warning("upload init failed (%d): %s", resp.errcode, resp.errmsg)
                return None, None
            ...
            for i, chunk in enumerate(chunk_list):
                ...
                if resp.errcode != 0:
                    self.logger.warning(
                        "upload chunk %d failed (%d): %s", i, resp.errcode, resp.errmsg
                    )
                    return None, None
            ...
            if resp.errcode != 0:
                self.logger.warning("upload finish failed (%d): %s", resp.errcode, resp.errmsg)
                return None, None
            ...
            self.logger.debug(
                "uploaded %s (%s) -> media_id=%s", fname, media_type, media_id[:16] + suffix
            )
    ...
    async def send(self, chat_id: str, message: ChannelMessage) -> None:
        ...
        try:
            ...
            is_progress = bool(message.metadata.get("_progress"))
            ...
            for file_path in message.media or []:
                ...
                if media_id:
                    ...
                    self.logger.debug("sent %s -> %s", media_type, chat_id)
                else:
            ...
            if frame:
                ...
                self.logger.info("proactive send to %s", chat_id)

        except Exception:
            self.logger.exception("Error sending message to chat_id=%s", chat_id)
