...
class MSTeamsConfig:
    ...
    ref_ttl_days: int = MSTEAMS_REF_TTL_DAYS
    prune_web_chat_refs: bool = True
    prune_non_personal_refs: bool = True
    ref_touch_interval_s: int = MSTEAMS_REF_TOUCH_INTERVAL_S
    trusted_service_url_hosts: list[str] = MSTEAMS_DEFAULT_TRUSTED_SERVICE_URL_HOSTS.copy()
    ...
    @classmethod
    def from_dict(cls, data: dict[str, Any]) -> MSTeamsConfig:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "mention_only_response": self.mention_only_response,
            "validate_inbound_auth": self.validate_inbound_auth,
            "ref_ttl_days": self.ref_ttl_days,
            "prune_web_chat_refs": self.prune_web_chat_refs,
            "prune_non_personal_refs": self.prune_non_personal_refs,
            "ref_touch_interval_s": self.ref_touch_interval_s,
            "trusted_service_url_hosts": self.trusted_service_url_hosts.copy(),
        }
...
class MSTeamsChannel(Channel):
    ...
    def default_config(cls) -> dict[str, Any]:
        return MSTeamsConfig().to_dict()

    def __init__(self, config: Any, manager: ChannelManager) -> None:
        if isinstance(config, dict):
            config = MSTeamsConfig.from_dict(config)
        ...
        # Migrate: get_workspace_path() → Path.home() / ".soothe" / "state"
        state_dir = Path.home() / ".soothe" / "state"
        self._refs_path = state_dir / "msteams_conversations.json"
    ...
    async def start(self) -> None:
        ...
        if not MSTEAMS_AVAILABLE:
            logger.error("PyJWT not installed. Run: pip install soothe-daemon")
            return

        if not self.config.app_id or not self.config.app_password:
            logger.error("app_id/app_password not configured")
            return
            ...
            def do_POST(self) -> None:
                ...
                if channel.config.validate_inbound_auth:
                    try:
                        ...
                        logger.warning("Inbound auth validation failed: %s", e)
                ...
                try:
                    ...
                    logger.warning("Activity handling failed: %s", e)
    ...
    async def send(self, chat_id: str, message: ChannelMessage) -> None:
        ...
        if not ref:
            raise RuntimeError(f"MSTeams conversation ref not found for chat_id={chat_id}")

        if not self._is_trusted_service_url(ref.service_url):
            raise RuntimeError(
                f"MSTeams conversation ref has untrusted service_url for chat_id={chat_id}"
            )
        ...
        try:
            ...
            logger.info("Message sent to %s", ref.conversation_id)
            self._touch_conversation_ref(str(chat_id), persist=True)
        except Exception:
    ...
    async def _handle_activity(self, activity: dict[str, Any]) -> None:
        ...
        if conversation_type and conversation_type not in ("personal", ""):
            logger.debug("Ignoring non-DM conversation %s", conversation_type)
            return
        ...
        # Migrate: use _handle_message which calls _manager.handle_inbound
    ...
    async def _validate_inbound_auth(self, auth_header: str, activity: dict[str, Any]) -> None:
        ...
        if not MSTEAMS_AVAILABLE:
            raise RuntimeError("PyJWT not installed. Run: pip install soothe-daemon")
    ...
    def _load_refs_raw(self) -> tuple[dict[str, Any], dict[str, Any], bool]:
        ...
        if self._refs_path.exists():
            try:
                ...
                logger.warning("Failed to load conversation refs: %s", e)
    ...
    def _prune_conversation_refs(self, *, now: float | None = None) -> bool:
        ...
        logger.info(
            "Pruned %d stale/unsupported conversation refs (ttl=%d days)",
            len(keys_to_drop),
            ttl_days,
        )
        return True
    ...
    def _save_refs_locked(self, *, prune: bool = True) -> None:
        ...
        try:
            ...
            logger.warning("Failed to save conversation refs: %s", e)
