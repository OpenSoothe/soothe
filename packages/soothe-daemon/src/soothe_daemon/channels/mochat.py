...
class MochatConfig:
    ...
    def __init__(
        ...
        mention: MochatMentionConfig | None = None,
        groups: dict[str, MochatGroupRule] | None = None,
        ...
        self.mention = mention or MochatMentionConfig()
        self.groups = groups or {}
        self.reply_delay_mode = reply_delay_mode
        self.reply_delay_ms = reply_delay_ms

    @classmethod
    def from_dict(cls, data: dict[str, Any]) -> MochatConfig:
        """Create config from dictionary."""
        mention_data = data.get("mention", {})
        mention = MochatMentionConfig(
            require_in_groups=mention_data.get("require_in_groups", False),
        )
        groups_data = data.get("groups", {})
        groups = {
            k: MochatGroupRule(require_mention=v.get("require_mention", False))
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "mention": {"require_in_groups": self.mention.require_in_groups},
            "groups": {k: {"require_mention": v.require_mention} for k, v in self.groups.items()},
...
class MochatChannel(Channel):
    ...
    def default_config(cls) -> dict[str, Any]:
        return MochatConfig().to_dict()

    def __init__(self, config: Any, manager: ChannelManager) -> None:
        if isinstance(config, dict):
            config = MochatConfig.from_dict(config)
        ...
        self._state_dir = Path.home() / ".soothe" / "mochat"
    ...
    async def start(self) -> None:
        ...
        if not self.config.claw_token:
            logger.error("[%s] claw_token not configured", self.name)
            return
    ...
    async def send(self, chat_id: str, message: ChannelMessage) -> None:
        ...
        if not self.config.claw_token:
            logger.warning("[%s] claw_token missing, skip send", self.name)
            return

        parts = [message.content.strip()] if message.content and message.content.strip() else []
        if message.media:
            parts.extend(m for m in message.media if isinstance(m, str) and m.strip())
        content = "\n".join(parts).strip()
        if not content:
            return

        target = resolve_mochat_target(chat_id)
        if not target.id:
            logger.warning("[%s] outbound target is empty", self.name)
            return
        ...
        try:
            if is_panel:
                await self._api_send(
                    ...
                    content,
                    message.reply_to,
                    self._read_group_id(message.metadata),
                )
            else:
                await self._api_send(
                    "/api/claw/sessions/send", "sessionId", target.id, content, message.reply_to
                )
        except Exception:
            logger.exception("[%s] Failed to send message", self.name)
            raise
    ...
    async def _start_socket_client(self) -> bool:
        if not SOCKETIO_AVAILABLE:
            logger.warning("[%s] python-socketio not installed, using polling fallback", self.name)
            return False
        ...
        if not self.config.socket_disable_msgpack:
            if MSGPACK_AVAILABLE:
                ...
                logger.warning(
                    "[%s] msgpack not installed but socket_disable_msgpack=false; using JSON",
                    self.name,
                )
        ...
        async def disconnect() -> None:
            if not self._running:
                return
            self._ws_connected = self._ws_ready = False
            logger.warning("[%s] websocket disconnected", self.name)
            await self._ensure_fallback_workers()

        @client.event
        async def connect_error(data: Any) -> None:
            logger.error("[%s] websocket connect error: %s", self.name, data)
        ...
        try:
            ...
            logger.exception("[%s] Failed to connect websocket", self.name)
            with suppress(Exception):
    ...
    async def _subscribe_sessions(self, session_ids: list[str]) -> bool:
        ...
        if not ack.get("result"):
            logger.error(
                "[%s] subscribeSessions failed: %s", self.name, ack.get("message", "unknown error")
            )
            return False
    ...
    async def _subscribe_panels(self, panel_ids: list[str]) -> bool:
        ...
        if not ack.get("result"):
            logger.error(
                "[%s] subscribePanels failed: %s", self.name, ack.get("message", "unknown error")
            )
            return False
        return True
    ...
    async def _refresh_loop(self) -> None:
        ...
        while self._running:
            ...
            try:
                ...
                logger.warning("[%s] refresh failed: %s", self.name, e)
    ...
    async def _refresh_sessions_directory(self, subscribe_new: bool) -> None:
        try:
            ...
            logger.warning("[%s] listSessions failed: %s", self.name, e)
            return
    ...
    async def _refresh_panels(self, subscribe_new: bool) -> None:
        try:
            ...
            logger.warning("[%s] getWorkspaceGroup failed: %s", self.name, e)
            return
    ...
    async def _session_watch_worker(self, session_id: str) -> None:
        while self._running and self._fallback_mode:
            try:
                ...
                logger.warning("[%s] watch fallback error (%s): %s", self.name, session_id, e)
    ...
    async def _panel_poll_worker(self, panel_id: str) -> None:
        ...
        while self._running and self._fallback_mode:
            try:
                ...
                logger.warning("[%s] panel polling error (%s): %s", self.name, panel_id, e)
    ...
    async def _load_session_cursors(self) -> None:
        ...
        try:
            ...
            logger.warning("[%s] Failed to read cursor file: %s", self.name, e)
            return
    ...
    async def _save_session_cursors(self) -> None:
        try:
            ...
            logger.warning("[%s] Failed to save cursor file: %s", self.name, e)
