"""Daemon-side ``goal_interrupted`` ledger marker for cancelled goals.

When a user submits a new goal (or cancels) while one is in flight, the daemon
runs **outside** the StrangeLoop graph and has only ``loop_id`` — not the
``LoopRuntimeContext`` that :func:`append_goal_interrupted_ledger_pair` needs.
This module is the persistence-side sibling: it loads the CE ledger + DAG,
appends a ``phase="goal_interrupted"`` Human+AI pair whose AI body is a compact
deterministic digest of the cancelled goal's partial ``execute_step`` work,
and persists both. The DAG goal status is left to
``QueryEngine._mark_active_context_goals_cancelled`` (which sets ``cancelled``);
this helper only adds the ledger marker so the next goal's planning projection
can bound the cancelled segment and surface what was done / what's left.
"""
