...
def test_image_data_from_bytes_rejects_oversized() -> None:
    raw = _tiny_png_bytes()
    with patch("soothe_cli.display.media_utils.MAX_MEDIA_BYTES", 8):
        assert image_data_from_bytes(raw) is None


def test_get_image_from_clipboard_returns_none_when_empty() -> None:
    with patch(
        "soothe_cli.display.media_utils._read_clipboard_image_bytes",
        return_value=None,
    ):
...
async def test_attach_clipboard_image_inserts_placeholder() -> None:
    ...
    with (
        ...
        patch(
            "soothe_cli.display.media_utils.get_image_from_clipboard",
            return_value=media,
        ),
    ):
        app = MagicMock()
...
def test_paste_command_registered() -> None:
    from soothe_cli.commands.command_registry import COMMANDS, SIDE_EFFECT_FREE
