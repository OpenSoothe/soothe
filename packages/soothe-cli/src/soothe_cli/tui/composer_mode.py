...
clarification questions to the operator unless they opt into Auto, Plan, or Ask.
"""
...
def resolve_composer_wire_fields(mode: str) -> ComposerWireFields:
    ...
    Returns:
        ...
        plus sticky ``planner``; ask mode sends ``clarification_mode=auto`` plus
        ``interaction_mode=ask``. Slash routing in the message still wins over
        the sticky hint at send time.
    """
    normalized = normalize_composer_mode(mode)
    if normalized == COMPOSER_MODE_PLAN:
        return ComposerWireFields(COMPOSER_MODE_AUTO, "planner", None)
