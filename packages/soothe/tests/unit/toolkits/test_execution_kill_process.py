...
from soothe.toolkits.execution import KillProcessTool, _kill_process_tree
...
def test_kill_process_not_found() -> None:
    tool = KillProcessTool()
    with patch("soothe.toolkits.execution.os.kill", side_effect=ProcessLookupError):
...
def test_kill_process_permission_denied_on_probe() -> None:
    tool = KillProcessTool()
    with patch("soothe.toolkits.execution.os.kill", side_effect=PermissionError):
        result = tool._run(42)
...
def test_kill_process_uses_process_group_teardown(monkeypatch) -> None:
    ...
    monkeypatch.setattr("soothe.toolkits.execution.os.kill", fake_kill)
...
def test_kill_process_signals_shutdown_when_process_survives(monkeypatch) -> None:
    ...
    monkeypatch.setattr("soothe.toolkits.execution._process_is_alive", lambda _pid: True)
...
def test_kill_process_tree_noop_for_invalid_pid() -> None:
    with patch("soothe.toolkits.execution.os.killpg") as mock_killpg:
