...
def test_protected_kill_refusal_ws_listener(monkeypatch) -> None:
    monkeypatch.setattr(
        "soothe.toolkits.execution._soothed_pid_from_pidfile",
        lambda: None,
    )
    monkeypatch.setattr(
        "soothe.toolkits.execution._pid_listening_on_port",
        lambda _port: 55555,
    )
...
def test_kill_process_tree_avoids_killpg_on_own_group(monkeypatch) -> None:
    ...
    monkeypatch.setattr("soothe.toolkits.execution.os.getpgid", lambda _pid: 77)
    monkeypatch.setattr(
        "soothe.toolkits.execution.os.kill",
    ...
    monkeypatch.setattr("soothe.toolkits.execution.os.killpg", boom)
...
def test_kill_process_tree_uses_killpg_for_foreign_group(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "soothe.toolkits.execution.os.killpg",
