...
class TestPostgresPoolRegistry:
    ...
    async def test_open_all_binds_three_pools(self) -> None:
        ...
        with (
            patch(
                "soothe.foundation.persistence.postgres_provisioning.ensure_postgres_databases_async",
            ...
            patch(
                "soothe.foundation.sloop.state.persistence.postgres_schema.initialize_agentloop_postgres_schema",
                new_callable=AsyncMock,
            ),
            patch(
                "soothe.foundation.persistence.db_init.initialize_database",
                new_callable=AsyncMock,
            ),
        ):
