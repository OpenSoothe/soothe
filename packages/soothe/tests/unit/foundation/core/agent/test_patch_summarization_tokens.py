...
from soothe.foundation.core.agent import _patch as patch_module


def test_tools_token_cache_key_is_stable() -> None:
    tools = [{"name": "ls", "description": "list files"}]
    assert patch_module._tools_token_cache_key(tools) == patch_module._tools_token_cache_key(tools)
...
def test_cached_tools_token_count_counts_once() -> None:
    ...
    first = patch_module._cached_tools_token_count(counter, [{"name": "a"}])
    second = patch_module._cached_tools_token_count(counter, [{"name": "a"}])
...
def test_split_conversation_token_count_adds_messages_and_tools() -> None:
    ...
    total = patch_module._split_conversation_token_count(
        counter,
