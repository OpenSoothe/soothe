...
from soothe.daemon.event_size_stats import (
...
def test_emit_logs_and_resets_window() -> None:
    ...
    lines: list[str] = []
    with patch("soothe.daemon.event_size_stats.time.monotonic", return_value=10.0):
    ...
    lines.clear()
    with patch("soothe.daemon.event_size_stats.time.monotonic", return_value=20.0):
        assert not c.emit_log_if_active(idle_pause_seconds=120.0, log_fn=lines.append)
    assert lines == []
...
def test_idle_discards_window_without_log() -> None:
    c = EventSizeDistributionCollector()
    with patch("soothe.daemon.event_size_stats.time.monotonic", return_value=0.0):
        c.record_event_dict({"type": "a"})
    lines: list[str] = []
    with patch("soothe.daemon.event_size_stats.time.monotonic", return_value=200.0):
        assert not c.emit_log_if_active(idle_pause_seconds=120.0, log_fn=lines.append)
    assert lines == []
    with patch("soothe.daemon.event_size_stats.time.monotonic", return_value=300.0):
        c.record_event_dict({"type": "b"})
    with patch("soothe.daemon.event_size_stats.time.monotonic", return_value=350.0):
        assert c.emit_log_if_active(idle_pause_seconds=120.0, log_fn=lines.append)
    assert len(lines) == 1
    assert "n=1" in lines[0]
