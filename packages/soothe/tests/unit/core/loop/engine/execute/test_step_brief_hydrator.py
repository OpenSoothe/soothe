...
from soothe.sloop.engine.execute.step_brief_hydrator import StepBriefHydration, StepBriefHydrator
...
async def test_hydrator_uses_llm_when_available(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "soothe.sloop.engine.execute.step_brief_hydrator.ainvoke_structured_traced",
        _fake_invoke,
    )
