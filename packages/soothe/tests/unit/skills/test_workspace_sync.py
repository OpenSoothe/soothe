...
def test_is_builtin_skill_directory() -> None:
    package_builtins = Path(__file__).resolve().parents[3] / "src" / "soothe" / "built_in_skills"
...
def test_collect_external_from_host_skill_roots(monkeypatch: MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "soothe.skills.workspace_sync.get_built_in_skills_paths",
        fake_builtin_paths,
    )
    cfg = SootheConfig()
...
def test_build_skill_context_omits_folder_for_builtin() -> None:
    builtin_path = next(
        (Path(__file__).resolve().parents[3] / "src" / "soothe" / "built_in_skills").iterdir()
    )
    meta = {
        "name": "builtin-test",
        "description": "d",
