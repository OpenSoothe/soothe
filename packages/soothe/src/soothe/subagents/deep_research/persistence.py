"""Persist deep_research reports under workspace ``.soothe/agents/deep_research/``."""
...
from soothe.utils.runtime import get_workspace_subagent_output_dir

from .display_summary import deep_research_brief_summary_for_display, derive_report_title

logger = logging.getLogger(__name__)


@dataclass(frozen=True)
class SavedDeepResearchReport:
    """Result of persisting a synthesized report."""
...
def slugify_report_title(title: str, *, max_len: int = 60) -> str:
    ...
    return slug[:max_len].strip("-") or "research-report"
...
def save_deep_research_report(
    ...
    *,
    ...
    reports_dir = get_workspace_subagent_output_dir("deep_research")
    host_path = _unique_report_path(reports_dir, slug)

    try:
        from soothe.toolkits._internal.backend_ops import backend_mkdir, backend_write_file

        backend_mkdir(reports_dir, soothe_config)
        backend_write_file(host_path, text, soothe_config)
    except Exception:
        logger.warning("[deep_research] failed to save report", exc_info=True)
        return None

    brief_summary = deep_research_brief_summary_for_display(text)
    display_path = _display_path_for(host_path)
    logger.info("[deep_research] saved report to %s", display_path)
    return SavedDeepResearchReport(
        host_path=host_path,
...
def format_saved_report_answer(saved: SavedDeepResearchReport) -> str:
    """Format the subagent tool result as summary plus pointer to the saved file."""
    return f"## Summary\n\n{saved.brief_summary}\n\nFull report saved to: `{saved.display_path}`"


__all__ = [
    "SavedDeepResearchReport",
