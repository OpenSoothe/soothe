"""Summaries for deep_research completion display."""
...
def _body_after_scope(report: str) -> str:
    ...
    next_heading = SECTION_HEADING_RE.search(remainder)
    if next_heading:
...
def derive_report_title(report: str, topic: str) -> str:
    ...
    for match in SECTION_HEADING_RE.finditer(report or ""):
    ...
    return cleaned[:120] if cleaned else "research-report"


def deep_research_brief_summary_for_display(report: str, *, max_len: int = 320) -> str:
...
def deep_research_report_summary_for_display(report: str, *, max_len: int = 160) -> str:
    ...
    return deep_research_brief_summary_for_display(report, max_len=max_len)
