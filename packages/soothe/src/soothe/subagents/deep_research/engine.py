"""deep_research engine — iterative public web research loop."""
...
if TYPE_CHECKING:
    ...
    from .effort import DeepResearchEffortProfile
    from .protocol import WebSearchSourceProtocol

logger = logging.getLogger(__name__)

_research_wire = ResearchWireEmitter(
    progress_event_type=DeepResearchProgressEvent,
    step_event_type=DeepResearchStepCompletedEvent,
    logger=logger,
)
...
def _get_shared_pool() -> ThreadPoolExecutor:
    ...
    if _shared_pool is None:
        _shared_pool = ThreadPoolExecutor(max_workers=4, thread_name_prefix="deep-research")
...
class DeepResearchEngineState(dict):
...
_PLAN_RESEARCH = """\
You are a public web research analyst. For the material below:
...
3. Generate targeted web search queries (< 50 chars, same language as the user ask).
...
_SUMMARIZE = """\
Summarise raw web results for "{topic}". Preserve source references.
...
def _effort_profile(state: dict[str, Any], config: DeepResearchConfig) -> DeepResearchEffortProfile:
...
def build_deep_research_engine(
    model: BaseChatModel,
    web_source: WebSearchSourceProtocol,
    config: DeepResearchConfig | None = None,
    *,
    synthesis_model: BaseChatModel | None = None,
    soothe_config: Any | None = None,
) -> Any:
    """Build and compile the deep_research LangGraph."""
    ...
    def plan_research_node(state: dict[str, Any]) -> dict[str, Any]:
        ...
        emit_subagent_wire_event(
            DeepResearchStartedEvent(topic_preview=str(topic)[:200], effort=effort).to_dict(),
            logger,
        )
        ...
        try:
            ...
            logger.warning("[deep_research] plan timed out, using fallback", exc_info=True)
            parsed = None
    ...
    def gather_node(state: dict[str, Any]) -> dict[str, Any]:
        ...
        async def _search() -> list[SourceResult]:
            try:
                return await asyncio.wait_for(
                    web_source.query(query, context),
                    timeout=cfg.source_timeout_sec,
                )
            except Exception:
                logger.debug("[deep_research] web search failed", exc_info=True)
                return []
        ...
        if not search_results:
            ...
            _emit_step("WebSearch", f"{query[:80]} → 0 hits", duration_ms=duration_ms)
            return {
                "search_summaries": [f"No web results for: {query}"],
        ...
        _emit_step("WebSearch", preview, duration_ms=duration_ms)

        summary_parts: list[str] = []
        source_refs: list[str] = []
        ref_dicts: list[dict] = []
        for r in search_results:
            summary_parts.append(f"[search] {r.content[:2000]}")
            source_refs.append(f"web_search:{r.source_ref}")
    ...
    def synthesize_node(state: dict[str, Any]) -> dict[str, Any]:
        # Never feed attached file bodies into synthesis — only web evidence.
