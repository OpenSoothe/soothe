"""JSON extraction helpers for Tacitus LLM nodes."""
...
def fallback_sub_questions(topic: str, *, domain: str = "public") -> list[dict[str, str]]:
    ...
    return [
        {
            "question": compact_search_query(topic, max_len=200),
    ...
    *,
    ...
    for sq in sub_questions or []:
        if isinstance(sq, dict):
            ...
            domain_hint = str(sq.get("suggested_domain", default_domain) or default_domain)
        else:
            question = str(sq).strip()
            domain_hint = default_domain
        if not question:
            continue
        queries.append(
            {
                "query": compact_search_query(question, max_len=120),
    ...
    return [
        {
            "query": compact_search_query(topic, max_len=120),
            "domain_hint": default_domain,
        }
    ]
