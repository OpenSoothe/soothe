...
- quiz: greetings, thanks, fillers, pleasantries, factual/trivia/definitions/simple math — any reply that needs no tools. quiz_response: short reply in the user's language (1-3 sentences). task_complexity=minimal.
...
2. Greeting, thanks, small talk, or factual question answerable without tools -> quiz
3. References prior conversation or follow-up on recent results -> continue_thread
4. Needs tools/files/analysis OR uncertain -> new_goal
...
intent_type: "continue_thread"|"new_goal"|"quiz". Set fields per type: quiz -> quiz_response (user's language; greetings or brief factual); continue_thread -> reuse_current_goal from active_goal, task_complexity medium; new_goal -> goal_description (5-15 words) + friendly_message (1-2 action-oriented sentences). task_complexity: minimal|simple|medium|complex.

Precedence: (1) new standalone or user ignores/resets prior -> new_goal (2) greeting/thanks/trivia without tools -> quiz (3) prior-turn follow-up -> continue_thread (4) tools needed or default -> new_goal.
