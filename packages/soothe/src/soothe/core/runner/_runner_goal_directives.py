...
class GoalDirectivesMixin:
    ...
    async def _synthesize_continuation(
        ...
        history = "\n".join(
            f"- Iteration {r.iteration}: {r.reflection_assessment[:100]}" for r in records[-5:]
        )
        plan_text = ""
        if plan:
            plan_text = f"\nRevised plan: {plan.goal}\nSteps: " + "; ".join(
                s.description for s in plan.steps[:5]
            )
    ...
    async def _apply_priority_directive(self, directive: Any) -> dict[str, Any]:
        ...
        logger.info(
            "Adjusted goal %s priority: %d -> %d", directive.goal_id, old_priority, goal.priority
        )
