...
class DreamingMode:
    ...
    async def _anticipate_goals(self) -> None:
        ...
        for goal_dir in sorted(
            runs_dir.rglob("goals/*/report.md"), key=lambda p: p.stat().st_mtime, reverse=True
        )[:5]:
            try:
        ...
        goals_text = "\n".join(
            f"- [{g['id']}] {preview_first(g['summary'], 200)}" for g in goal_summaries[:5]
        )
