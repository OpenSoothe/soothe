...
Usage:
    ...
    from soothe_nano.events import GOAL_CREATED, BRANCH_CREATED

    # For type-safe event emission (recommended)
    from soothe_nano.events import GoalCreatedEvent, custom_event
    yield custom_event(GoalCreatedEvent(goal_id=gid).to_dict())

    # For event registration
    from soothe_nano.events import register_event, EventPriority
...
def __getattr__(name: str) -> object:
    ...
    if name == "InternalEventBus":
        from soothe_nano.events.internal_bus import InternalEventBus

        return InternalEventBus
    # RFC-222: Internal event types (lazy import to avoid circular deps)
    if name == "INTERNAL_EVENT_TYPES":
        from soothe_nano.events.internal_events import INTERNAL_EVENT_TYPES  # noqa: F401

        return INTERNAL_EVENT_TYPES
    if name == "is_internal_event_type":
        from soothe_nano.events.internal_events import is_internal_event_type

        return is_internal_event_type

    # Internal goal events
    if name == "InternalGoalCompletedEvent":
        from soothe_nano.events.internal_events import InternalGoalCompletedEvent

        return InternalGoalCompletedEvent
    if name == "InternalGoalFailedEvent":
        from soothe_nano.events.internal_events import InternalGoalFailedEvent

        return InternalGoalFailedEvent
    if name == "InternalGoalProgressEvent":
        from soothe_nano.events.internal_events import InternalGoalProgressEvent

        return InternalGoalProgressEvent
    if name == "InternalGoalStateChangedEvent":
        from soothe_nano.events.internal_events import InternalGoalStateChangedEvent

        return InternalGoalStateChangedEvent
    if name == "InternalGoalsReadyEvent":
        from soothe_nano.events.internal_events import InternalGoalsReadyEvent

        return InternalGoalsReadyEvent

    # Internal loop events
    if name == "InternalLoopAssignedEvent":
        from soothe_nano.events.internal_events import InternalLoopAssignedEvent

        return InternalLoopAssignedEvent
    if name == "InternalLoopIdleEvent":
        from soothe_nano.events.internal_events import InternalLoopIdleEvent

        return InternalLoopIdleEvent
    if name == "InternalLoopReleasedEvent":
        from soothe_nano.events.internal_events import InternalLoopReleasedEvent

        return InternalLoopReleasedEvent
    if name == "InternalLoopSpawnedEvent":
        from soothe_nano.events.internal_events import InternalLoopSpawnedEvent

        return InternalLoopSpawnedEvent

    # Internal file events
    if name == "InternalFileLockedEvent":
        from soothe_nano.events.internal_events import InternalFileLockedEvent

        return InternalFileLockedEvent
    if name == "InternalFileReleasedEvent":
        from soothe_nano.events.internal_events import InternalFileReleasedEvent

        return InternalFileReleasedEvent
    if name == "InternalFileConflictEvent":
        from soothe_nano.events.internal_events import InternalFileConflictEvent

        return InternalFileConflictEvent

    # Internal autopilot events
    if name == "InternalAutopilotStartedEvent":
        from soothe_nano.events.internal_events import InternalAutopilotStartedEvent

        return InternalAutopilotStartedEvent
    if name == "InternalAutopilotStoppedEvent":
        from soothe_nano.events.internal_events import InternalAutopilotStoppedEvent

        return InternalAutopilotStoppedEvent
    if name == "InternalLoopPoolChangedEvent":
        from soothe_nano.events.internal_events import InternalLoopPoolChangedEvent

        return InternalLoopPoolChangedEvent
    if name == "InternalAutopilotDreamingEvent":
        from soothe_nano.events.internal_events import InternalAutopilotDreamingEvent

        return InternalAutopilotDreamingEvent
    if name == "InternalAutopilotAwakeEvent":
        from soothe_nano.events.internal_events import InternalAutopilotAwakeEvent
