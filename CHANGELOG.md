...
### Fixed
- Drop hard `typer<1.0.0` cap in `soothe-cli` and `soothe-daemon` (now `<2.0.0`); `pip install -U 'soothe[all]' soothe-cli soothe-daemon` no longer ResolutionImpossible on envs where `huggingface-hub` / `transformers` resolve `typer>=1.0`.
- `uv sync` / `make sync`: bump `langgraph-checkpoint-postgres` to `>=3.1.0,<4.0.0` (compatible with `langgraph-checkpoint` 4.x); re-add `community` as a workspace member with `soothe-sdk = { workspace = true }`; cap workspace `requires-python` to `<3.15` so uv does not resolve phantom 3.15 splits.

### Changed
- Promote `browser_use` and `claude` subagents from `soothe-community` to built-in `soothe.subagents.{browser_use,claude}` (trust_level `built-in`, registered via `soothe.plugins` entry points). Heavy deps move to opt-in extras `soothe[browser_use]` / `soothe[claude]`. Daemon `loop_gc.cleanup_claude_sessions` import updated; `soothe-community` drops these entry points and the `browser-use` / `anthropic` / `claude-agent-sdk` deps. Tests relocated to `packages/soothe/tests/unit/subagents/{browser_use,claude}/`.
- Align workspace floors to `0.5.25`: `soothe`, `soothe-cli`, `soothe-daemon` now require `soothe-sdk>=0.5.25,<1.0.0`; `soothe-daemon` requires `soothe>=0.5.25,<1.0.0` and (dev) `soothe-cli>=0.5.25`.
- `soothe-community`: bring deps in line with `soothe` (`langgraph>=1.1.1,<2.0.0`, `anthropic>=0.96.0,<1.0.0`, `soothe-sdk>=0.5.25,<1.0.0`); replace `<=0.12.0` on `browser-use` with `<0.13.0`; add upper caps on previously unbounded floors (`arxiv`, `sentence-transformers`, `pyzotero`, `scikit-learn`, `numpy`, `tiktoken`, `feedparser`, `claude-agent-sdk`).
- Add upper caps to previously unbounded deps: `aiohttp<4`, `aiosqlite<1`, `anyio<5`, `pexpect<5`, `requests<3`, `psycopg<4`, `langgraph-checkpoint-postgres<4` (in `soothe`); `fastapi<1`, `psutil<8`, `uvicorn<1` (in `soothe-daemon`); `textual<10`, `rich<15`, `pillow<12`, `websockets<16` (in `soothe-cli`).
