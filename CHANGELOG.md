...
### Added
- Sobo apps (Electron shell `apps/sobo`, QQ-style web UI `apps/soothe-web`, protocol bridge `apps/soothe-bridge`): the bridge maps an oRPC-compatible subset (`bootstrap`, `bots.*`, `threads.*`, `models.list`) onto Soothe daemon WebSocket methods (`loop_new`, `loop_input`, `loop_messages`, `loop_events`), serving the web UI statics on the same port. The bot↔loop ID mapping persists in SQLite via `node:sqlite` (`$SOOTHE_HOME/bridge/bridge.db`, overridable with `SOOTHE_BRIDGE_DATA_DIR`). The web UI is a custom contact-list + message-panel layout (deliberately not a fork of Rakazo's web app; only `@rakazo/contracts` wire shapes are referenced). Contact panel supports per-bot last-message previews, double-click rename, and delete; SSE events drive debounced history refetch.
...
- Update `docs/soothe-teammates-integration.md` to reflect implemented reality: Sobo naming and ports, custom QQ-style web UI replacing the Rakazo web fork (§7.2 decision record), bridge component status, `node:sqlite` store, and dependency graph without `@rakazo/ui-tokens`.
