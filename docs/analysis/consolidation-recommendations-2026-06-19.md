...
|-----------|----------|---------------|-------------------|
| `LoopState` | `soothe/foundation/loop/state/schemas.py` | StrangeLoop execution state | iteration, tokens, plan history, thread_id |
| `ThreadState` | `soothe_daemon/runtime/thread_state.py` | Per-checkpoint daemon isolation | thread_id, workspace, last_activity |
| `Checkpoint` models | `soothe/foundation/loop/state/checkpoint.py` | StrangeLoop checkpoint persistence | thread_id, goal_history, working_memory |
...
| `LoopMessage` utils | `soothe/foundation/loop/utils/messages.py` | StrangeLoop messages | content, phase, ledger recording |
