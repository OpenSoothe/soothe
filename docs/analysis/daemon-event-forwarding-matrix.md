...
| `stream_event` / `mode="messages"` loop-tagged AI with `phase` in `{goal_completion, quiz, autonomous_goal, direct_model}` | `_runner_agentic.py` forwards tagged assistant chunks from AgentLoop (`stream_event`) | Yes (subject to client `output_streaming` display policy) | User-visible answer text path (IG-317); not emitted as `soothe.output.goal_completion.*` |
