...
| --- | --- | --- | --- |
| `soothe.cognition.strange_loop.started` | `_runner_agentic.py::_run_agentic_loop` (`StrangeLoopStartedEvent`) | Yes | N/A |
| `iteration_started` (raw StrangeLoop internal) | `strange_loop.py::run_with_progress` -> `_runner_agentic.py` branch | No | Internal debug-only; runner logs only, does not `yield` |
| `plan_decision` (raw StrangeLoop internal) | `strange_loop.py::run_with_progress` -> `_runner_agentic.py` branch | No | Internal debug-only; runner logs only, does not `yield` |
| `soothe.cognition.strange_loop.step.started` | `_runner_agentic.py` maps `step_started` -> `StrangeLoopStepStartedEvent` | Yes | N/A |
| `soothe.cognition.strange_loop.step.completed` | `_runner_agentic.py` maps `step_completed` -> `StrangeLoopStepCompletedEvent` | Yes | N/A |
| `stream_event` carrying `ToolMessage` | `strange_loop.py` emits `stream_event`; `_runner_agentic.py::_forward_messages_chunk_for_tool_ui` | Yes | N/A |
...
| `stream_event` / `mode="messages"` loop-tagged AI with `phase` in `{goal_completion, quiz, autonomous_goal, direct_model}` | `_runner_agentic.py` forwards tagged assistant chunks from StrangeLoop (`stream_event`) | Yes (subject to client `output_streaming` display policy) | User-visible answer text path (IG-317); not emitted as `soothe.output.goal_completion.*` |
...
| `soothe.cognition.strange_loop.reasoned` | `_runner_agentic.py` maps `plan` -> `LoopAgentReasonEvent` | Conditional Yes | Suppressed when `status=="done"` and `next_action=="Goal achieved successfully"` |
| `iteration_completed` (raw StrangeLoop internal) | `strange_loop.py::run_with_progress` -> `_runner_agentic.py` branch | No | Internal debug-only; runner logs only, does not `yield` |
| `fatal_error` (raw StrangeLoop internal) | `strange_loop.py::run_with_progress` can emit `fatal_error`; `_runner_agentic.py` has no handling branch | No | Dropped due to missing forward mapping in runner |
| `soothe.cognition.strange_loop.completed` | `_runner_agentic.py` in `completed` branch (`StrangeLoopCompletedEvent`) | Yes | N/A |
