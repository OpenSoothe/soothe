...
## Goal
Move the `browser_use` and `claude` subagents from `soothe-community` into the core `soothe` package, gated behind opt-in extras so heavy deps are not pulled into the base install.

## Files touched
- New: `packages/soothe/src/soothe/subagents/browser_use/` (moved from `community/src/soothe_community/browser_use/`)
- New: `packages/soothe/src/soothe/subagents/claude/` (moved from `community/src/soothe_community/claude/`)
...
- Edit: `packages/soothe-daemon/src/soothe_daemon/loop_gc.py` — `soothe_community.claude.session_bridge` → `soothe.subagents.claude.session_bridge`.
...
- **Path helper**: `soothe_community._paths.expand_path` → `soothe.utils.path.expand_path` (the helper was a verbatim mirror of that utility).

## Done when
- `./scripts/verify_finally.sh` passes.
- `grep -r soothe_community.browser_use\|soothe_community.claude packages docs CHANGELOG.md` returns nothing.
