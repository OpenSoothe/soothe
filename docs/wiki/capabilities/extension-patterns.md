...
|-------|----------|-------------|
| `built-in` | Core capabilities (plan, tacitus, browser_use, skillify, veritas) | Full — can request any permission |
...
- **Built-in plugins are immutable**: you cannot override `plan`, `tacitus`, `browser_use`, `skillify`, `veritas`, etc. with a lower-priority plugin of the same name. Use a different name.
