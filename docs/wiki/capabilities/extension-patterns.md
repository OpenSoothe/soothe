...
|-------|----------|-------------|
| `built-in` | Core capabilities (explore, plan, tacitus) | Full — can request any permission |
...
- **Built-in plugins are immutable**: you cannot override `explore`, `plan`, `tacitus`, etc. with a lower-priority plugin of the same name. Use a different name.
