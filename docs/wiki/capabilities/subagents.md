...
- **Report delivery** — `config.save_reports` (default `false`) returns the full report inline; set `true` to write under `.soothe/agents/deep_research/` and return a short summary + path.
