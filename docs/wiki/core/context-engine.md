...
- `absorb_guidance(goal_id, text, scope=, *, source=)` — async; accumulates operator guidance for the next autopilot dispatch (prefer `soothe.autopilot.intake.absorb_*` for typed intake).
