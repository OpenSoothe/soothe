...
| Wired-subagent route | When Pass 2 or slash `preferred_subagent` resolves to `planner`, `browser_use`, `deep_research`, or `academic_research`, StrangeLoop sets `intent_route=wired_subagent` and runs `invoke_wired_subagent` → execute → `goal_completion` (skip plan assess/generate). | RFC-630 |
