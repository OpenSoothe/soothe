...
| StrangeLoop `plan_generate` | StrangeLoop **planning-stage** node that drafts/refines the multi-step `AgentDecision` for the current goal wave. Not the intake `planner` subagent. | RFC-220, RFC-227 |
| StrangeLoop `plan_assess` | StrangeLoop **planning-stage** node that assesses progress / continuation before (or instead of) regenerating a plan. Not the intake `planner` subagent. | RFC-220, RFC-226 |
