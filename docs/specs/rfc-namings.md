...
| Term | Definition | Introduced In |
|------|------------|---------------|
| Intake LLM | Two-pass fast-model classification: Pass 1 social vs task, Pass 2 scope (`trivial \| simple \| complex`) plus optional `wire_subagent`. Runs overlapped with pre-graph IO. Replaces the binary `IntentClassifier` LLM + its `_is_likely_agentic` heuristic bypass. | RFC-630 |
| Wired-subagent route | When Pass 2 or slash `preferred_subagent` resolves to an allowlisted specialist, StrangeLoop sets `intent_route=wired_subagent` and runs `invoke_wired_subagent`. Catalog `planner` continues resolve → execute → `goal_completion`. Intake-only specialists (`browser_use`, `deep_research`, `academic_research`) direct-invoke then `goal_completion` (no execute), with stream/orphan-card UX per RFC-630 §6.3.3 and RFC-628 Part III. | RFC-630 |
| Intake-only subagent | Specialist registered for wired invoke but omitted from the open CoreAgent `task` catalog and plan `delegate` surface (`browser_use`, `deep_research`, `academic_research`). `planner` stays dual-exposed. | RFC-630 |
...
| `wired_subagent_started` / `completed` / `failed` / `cancelled` | StrangeLoop lifecycle envelopes for the intake-only direct invoke; TUI mounts and completes the orphan SubAgent card. Wire progress is forwarded `soothe.subagent.*` customs stamped with `invocation_id`. | RFC-630 |
...
| Branch routing | The start-phase dispatch paths: `quiz`→END, fresh `trivial`→synthetic 1-step plan (no plan LLM), fresh `simple`→lightweight `plan_generate`, `complex`→the full existing spine with the IG-476 fresh-loop skip intact; continuation `trivial/simple` first route through `plan_assess` discriminator. | RFC-630 |
...
| Trivial-branch plan | Minimal 1-step `PlanResult` injected by `init_or_resume` for the `trivial` label: step action = the intake LLM's `goal_description` (no prefix), plan reasoning = `None` (no synthetic prose), soft direct-answer `expected_output`, and `requires_tool_use` from Pass 2 for the execute deliverable gate (IG-569). | RFC-630 |
