...
- `agent.clarification.*` and `agent.veritas.*` configuration additions in both `config.template.yml` and `config.dev.yml`.
...
| `config/config.template.yml`, `config/config.dev.yml` | New `agent.clarification.*` and `agent.veritas.*` sections |
...
Per project rule, both `config.template.yml` and `config.dev.yml` are updated in the same change.
