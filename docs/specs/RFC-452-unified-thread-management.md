...
The `soothe thread continue` command requires a running daemon for thread continuation. The deprecated `soothed attach --thread-id` has been replaced by `soothe thread continue`, which now exclusively operates via the daemon. See user documentation for complete CLI reference.
