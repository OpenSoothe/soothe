...
2. **Tool heads.** For each successful step's `step_messages`, walk forward and collect at most 8 (tool name, first-non-empty-line) pairs in arrival order. The tool name comes from the `AIMessage.tool_calls[*].name` matched to the next `ToolMessage`; the head comes from the matched `ToolMessage.content`. Each `head` is `.strip().splitlines()[0][:120]`. Empty heads are kept (`head=""`) to preserve the tool-name signal.
3. **Evidence excerpts.** For each successful step, take the final AI text (same string already written to the ledger) and keep `text.strip()[:200]`. Dedupe by 64-char prefix. Cap the list at 3 most-recent entries.
