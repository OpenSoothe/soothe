...
Eliminate all `soothe.ux` imports from `soothe.daemon`. Shared text filtering, AI message extraction, Rich plan rendering, and slash-command handling live under `soothe` (merged from former `soothe.text` / `soothe.slash_commands`) and `soothe.plan`; UX re-exports where needed.
...
- `daemon/query_engine.py`: imports from `soothe` only (for text helpers).
