...
def validate_docker_yml(workflow: dict) -> list[str]:
    ...
    build_job = jobs.get('build-and-push') or jobs.get('build') or list(jobs.values())[0] if jobs else {}
    if 'if' in build_job:
        cond = build_job['if']
        ...
        if 'metadata' in step_name.lower() or 'tags' in step_name.lower():
            ...
            if tags:
                if 'publish_version_tags' in tags:
                    print(f"  ✓ Version tags conditional: present")
                if 'publish_branch_tags' in tags:
                    print(f"  ✓ Branch tags conditional: present")
...
def validate_release_docker_yml(workflow: dict) -> list[str]:
    ...
    if 'workflow_run' not in trigger_names:
        ...
        if wr.get('workflows') != ['Release Soothe Packages']:
            errors.append(f"ERROR: workflow_run should trigger on 'Release Soothe Packages', got: {wr.get('workflows')}")
        if wr.get('types') != ['completed']:
            errors.append(f"ERROR: workflow_run should trigger on 'completed', got: {wr.get('types')}")
